...
int main(int argc, const char **argv) {
	...
	std::shared_ptr<ClientEngine> client_engine (new ClientEngine(graphics_engine));
	std::shared_ptr<ControllerEngine> controller_engine (new ControllerEngine(client_engine));

	std::shared_ptr<Calendar> cal_graphics (new Calendar(30, graphics_engine));
	std::shared_ptr<Calendar> cal_client (new Calendar(1, client_engine));
	std::shared_ptr<Calendar> cal_controller (new Calendar(60, controller_engine));

	scheduler->add_calendar(cal_controller);
	scheduler->add_calendar(cal_client);
